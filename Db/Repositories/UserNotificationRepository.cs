...
namespace devices.Db.Repositories
{
    public class UserNotificationRespository
    {
        ...
        public async Task InsertUserDeviceAsync(UserDevice uDev)
        {
            try {
                ...
                foreach (Device dev in uDev.Devices) {
                    cmd.CommandText = @"INSERT INTO Devices (deviceId, active) VALUES (@deviceId, 1);";
                    cmd.Parameters.Add(new MySqlParameter
                    {
        ...
        {
            var cmd = Db.Connection.CreateCommand() as MySqlCommand;
            cmd.CommandText = @"SELECT user.id, user.username, device.id, device.deviceId, device.active FROM UsersDevices udev  
                    ...
                    while (await reader.ReadAsync())
                    {
                        if (user == null) 
                        {
                            ...
                            devicesList.Add(new Device()
                            {
                                ...
                                //Active = Convert.ToBoolean(await reader.GetFieldValueAsync<int>(4)),
                            });
                        }
                    }
                }
